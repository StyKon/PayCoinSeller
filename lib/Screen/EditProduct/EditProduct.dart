...
//========================= InIt MEthod ========================================
  ...
  void initState() {
    ...
    editProvider!.cityScrollController.addListener(_scrollListener);
    getAllData();
  ...
  _scrollListener() async {
    if (editProvider!.cityScrollController.offset >=
            editProvider!.cityScrollController.position.maxScrollExtent &&
        !editProvider!.cityScrollController.position.outOfRange) {
      if (mounted) {
        setState(() {});
...
//========================= Simple Product =====================================
    ...
    if (editProvider!.productType == "simple_product") {
      ...
      if (widget.model!.prVarientList![widget.model!.selVarient!].stockType !=
          null) {
        editProvider!.simpleproductStockStatus =
            widget.model!.prVarientList![widget.model!.selVarient!].stockType;
      }
      // for save setting
    ...
    if (editProvider!.productType == "variable_product") {
      ...
      if (widget.model!.stockType == "2") {
        ...
        int i = 0;
        for (var element in editProvider!.variationList) {
          i = i + 1;
        }
        editProvider!.col = colCount.length;
        editProvider!.row = widget.model!.prVarientList!.length;
              ...
              itemBuilder: (context, i) {
                return InkWell(
                  child: Stack(
                    ...
                    children: [
                      Image.network(
                        editProvider!.variationList[pos].imagesUrl![i],
                        width: 100,
                        height: 100,
                      ),
                      Container(
                  ...
                  onTap: () {
                    if (mounted) {
                      setState(
                        () {
                          editProvider!.variationList[pos].imagesUrl!
                              .removeAt(i);
                        },
                      );
                    }
...
//=========================== Body Part ========================================
  ...
  bool validateAndSave() {
    ...
    form.save();
    if (form.validate()) {
      if (editProvider!.productType == null) {
        setSnackbar(
          getTranslated(context, "Please select product type")!,
          context,
        );
        return false;
      } else if (editProvider!.productImage == '' &&
